...
namespace ETradeStudy.Application.Features.Queries.Product.GetAllProduct
{
    public class GetAllProductQueryHandler : IRequestHandler<GetAllProductQueryRequest, GetAllProductQueryResponse>
    {
        ...
        public async Task<GetAllProductQueryResponse> Handle(GetAllProductQueryRequest request, CancellationToken cancellationToken)
        {
            ...
            var products = _productRead.GetAll(false).Skip(request.Page * request.Size).Take(request.Size).OrderBy(p => p.ProductName).Select(p => new { p.Id,p.ProductName, p.Price, p.Stock, p.CreatedDate, p.UpdateDate });
